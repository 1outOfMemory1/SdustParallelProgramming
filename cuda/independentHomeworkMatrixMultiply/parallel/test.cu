...
{
    int i = threadIdx.x; //threadIdx.x 确定你在一行上的那一个
    c[i] = b[i] - a[i];
}
...
{
    int i = threadIdx.x + threadIdx.y*blockDim.x; //threadIdx.y*blockDim.x 确定你在那一个行上  threadIdx.x确定你在一行上的那一个上
    c[i] = b[i] - a[i];
}
...
__global__ void testThread3(int *c, const int *a, const int *b)
{
    ...
    //threadIdx.z*blockDim.x*blockDim.y 确定你在三维空间的哪一个面上  threadIdx.y*blockDim.x 确定你在那一个行上  threadIdx.x确定你在一行上的那一个上
    c[i] = b[i] - a[i];
}
...
{
    int i = threadIdx.x + blockDim.x*blockIdx.x; //blockDim.x 是常数 因为 block里边的线程是一维的 每次隔一个block的距离就可以找到下一个block中相应位置的值
    c[i] = b[i] - a[i];
}
